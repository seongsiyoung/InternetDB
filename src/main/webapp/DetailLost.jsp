        ...
        <%
            ...
            if(session.getAttribute("id") != null) {
                if(session.getAttribute("id").equals(user_id)) {
                    out.println("<td colspan='2'><br><div class='ModifyAndDelete'><button id='mdBtn' onclick=\"location.href='modifyLost.jsp?lost_id=" + lost_id + "'\">수정</button>");
                    out.println("<button id='mdBtn' onclick=\"location.href='deleteLostItem.jsp?lost_id=" + lost_id + "'\">삭제</button></div></td></tr>");
                }
            }
        %>
    </table>
    ...
    <div class="commentWrite" align="center">
        <form method="post" action="saveComment.jsp" class="commentForm">
            <%
                ...
                // 로그인 되지 않은 상태이면 댓글 작성란이 보이지 않도록 하는 코드
                if(session.getAttribute("id") != null) {
